...
public class RandomizerApp {
    ...
    private void initialize() {
        ...
        generalRandPanel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"),
        ...
        generalRandKeepPokeSpecMovesBox.setToolTipText(
                "Some moves are specific to a specific pokemon like Call for Family. Checking this will keep those types of moves on the same pokemon. Note that if there are multiple versions of the card it, it can go with any of them");
        generalRandPanel.add(generalRandKeepPokeSpecMovesBox);
        generalRandKeepPokeSpecMovesBox.setEnabled(false);

        generalRandKeepTypeSpecMovesBox.setToolTipText(
                "Some moves are Energy type specific (such as ember requiring a Fire Energy discard). This will keep it so they will only be allowed for the Energy type that matches their effect");
        ...
        moveRandPanel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"),
                "Attacks", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
        ...
        moveRandUnchangedButton.addItemListener(event -> {
            if (event.getStateChange() == ItemEvent.SELECTED) {
                generalRandNumMovesBox.setEnabled(false);
            } else if (event.getStateChange() == ItemEvent.DESELECTED
                    && (pokePowerIncludeWithMovesBox.isSelected()
                            || !pokePowerUnchangedButton.isSelected())) {
        ...
        moveRandTypePanel.setBorder(
                new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Attack Type Changes",
        ...
        pokePowerPanel.setBorder(new TitledBorder(null, "Poke Powers", TitledBorder.LEADING,
        ...
        pokePowerUnchangedButton.addItemListener(event -> {
            ...
            if (!pokePowerIncludeWithMovesBox.isSelected()) {
                ...
                // If both this and the attacks are not unchanged, enable move number randomization
                        ...
                        && !moveRandUnchangedButton.isSelected()) {
                    generalRandNumMovesBox.setEnabled(true);
                }
            }
        });
        ...
        pokePowerIncludeWithMovesBox.addItemListener(event -> {
            if (event.getStateChange() == ItemEvent.SELECTED) {
                pokePowerUnchangedButton.setEnabled(false);
                pokePowerShuffleButton.setEnabled(false);
                pokePowerRandomButton.setEnabled(false);
        ...
        moveTypesPanel.setBorder(new TitledBorder(null, "Move Types", TitledBorder.LEADING,
                TitledBorder.TOP, null, null));
        ...
        moveTypesRandomPanel.setBorder(
