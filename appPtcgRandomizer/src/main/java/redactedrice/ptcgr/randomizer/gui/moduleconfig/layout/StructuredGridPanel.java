...
public final class StructuredGridPanel extends JPanel implements ArgumentValueEditor {
    ...
    private StructuredGridCollectionNode root;
    ...
    private void applyEditable(StructuredGridCollectionNode node) {
        for (StructuredGridEntry entry : node.entries) {
    ...
    private void rebuild() {
        removeAll();
        root = new StructuredGridCollectionNode(type);
    ...
    private int renderCollection(StructuredGridCollectionNode node, List<Object> rawEntries, int colOffset,
        ...
        for (int i = 0; i < rawEntries.size(); i++) {
            ...
            StructuredGridEntry entry = new StructuredGridEntry();
            ...
            if (childType.isComplex()) {
                ...
                StructuredGridCollectionNode childNode = new StructuredGridCollectionNode(childType);
    ...
    private void removeEntry(StructuredGridCollectionNode node, StructuredGridEntry entry) {
    ...
    private void applyStructuralChange(StructuredGridCollectionNode target, Consumer<List<Object>> mutation) {
        try {
    ...
    private List<Object> extractRawApplying(StructuredGridCollectionNode node, StructuredGridCollectionNode target,
