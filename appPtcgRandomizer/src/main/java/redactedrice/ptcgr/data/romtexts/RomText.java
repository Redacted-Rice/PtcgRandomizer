...
public class RomText {
    public static final char SPECIAL_SYM_RESERVE_SPACE_CHAR = 0x11; // Device control 1 for no
                                                                    // particular reason
    ...
    public RomText(int maxCharsPerLine, int preferredLinesPerBlock, int maxLinesPerBlock,
            int maxBlocks) {
        this(CharSetPrefix.HALF_WIDTH, maxCharsPerLine, preferredLinesPerBlock, maxLinesPerBlock,
                maxBlocks);
    }

    public RomText(CharSetPrefix charSet, int maxCharsPerLine, int preferredLinesPerBlock,
            int maxLinesPerBlock, int maxBlocks) {
    ...
    protected List<String> formatText(String text) {
        List<String> formatted = StringUtils.prettyFormatText(text, maxCharsPerLine,
                maxLinesPerBlock, preferredLinesPerBlock, maxBlocks);
    ...
    public void readDataAndConvertIds(byte[] bytes, int[] textIdIndexes, Texts idsToText) {
        if (textIdIndexes.length != maxBlocks) {
            throw new IllegalArgumentException(
                    "Reading rom text was passed the wrong number of id indexes :"
    ...
    public void writeTextId(byte[] bytes, int[] indexesToWriteAt) {
        if (indexesToWriteAt.length != maxBlocks) {
            throw new IllegalArgumentException(
                    "Writing rom text was passed the wrong number of id indexes :"
                            + indexesToWriteAt.length);
        }

        if (textBlockIds.size() != textBlocks.size()) {
            throw new IllegalArgumentException(
                    "Object not setup to write! The found number of ids (" + textBlockIds.size()
                            + ") does not match the number of text blocks (" + textBlocks.size()
        ...
        if (textBlocks.size() > expectedBlocks) {
            throw new IllegalArgumentException(
                    "Too many text blocks passed in! Expected " + expectedBlocks + " got "
                            + textBlocks.size() + ". First block is \"" + textBlocks.get(0) + "\"");
        }

        // Write each blocks id
        int blockIndex = 0;
        for (; blockIndex < textBlockIds.size(); blockIndex++) {
            ByteUtils.writeAsShort(textBlockIds.get(blockIndex).shortValue(), bytes,
                    indexesToWriteAt[blockIndex]);
        }
    ...
    private static String reserveSpaceForSpecialChars(String text) {
        // Energy types behave a bit oddly - if there is a space before them (which there always
        // seems
        // to be is) then they need to align with an even char position. If the space is an even
        // char,
        ...
        for (CharMapConstants.SpecialSymbol specialSym : CharMapConstants.SpecialSymbol.values()) {
            ...
            text = text.replaceAll(specialSym.getString(),
                    specialSym.getString() + SPECIAL_SYM_RESERVE_SPACE_CHAR);
        }
        return text;
    }
