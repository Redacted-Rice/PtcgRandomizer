...
public class CustomCardEffect extends CardEffect {
    ...
    public static void addTweakToAllowEffectsInMoreBanks(Blocks blocks, InstructionParser parser) {
        ...
        UnconstrainedMoveBlock logicBlock = new UnconstrainedMoveBlock(
                ...
                (byte) 0x1); // Try to fit it in home to avoid bankswaps
    ...
    public void addEffectFunction(EffectFunctionTypes type, List<String> sourceLines,
        ...
        effects.put(type, new EffectFunction(new UnconstrainedMoveBlock(
                new ParserCodeBlock(sourceLines, parser), effectFunctionPrefs)));
    }
        ...
        if (!effects.isEmpty()) {
            ...
            for (Entry<EffectFunctionTypes, EffectFunction> effect : effects.entrySet()) {
                ...
                } else // Otherwise we need to figure out where to put it
                {
                    ...
                    effectCommand.appendInstruction(new EffectFunctionPointer(effect.getKey(),
                            effect.getValue().getBlock().getId()));
                }
            }
