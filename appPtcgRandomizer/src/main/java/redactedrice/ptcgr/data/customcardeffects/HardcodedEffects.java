...
public class HardcodedEffects {
    public enum MoveUniqueness {
        GLOBALLY, PER_ENERGY_TYPE, PER_CARD_NAME, PER_CARD_ID,
    }
    ...
    public static class CallForFamily {
        ...
        public static CustomCardEffect createMoveEffect(
                /* Cards<Card> cards, */ CardGroup<Card> basics, InstructionParser parser) {
            // Just assume the first one for now. This probably won't work well for cards where
            // there are versions of them
            ...
            List<String> playerSelectCode = Arrays.asList("CallForFamilyPS" + cardName + ":",
                    ...
                    ".loop", "bank1call " + FUNC_DISPLAY_CARD_LIST_BANK1ADDR, "jr c, .pressed_b",
                    "call " + FUNC_GET_CARD_ID_FROM_DECK_INDEX_ADDR, "ld bc, " + cardId,
                    ...
                    "call " + FUNC_GET_TURN_DUELIST_VAR_ADDR, ".loop_b_press", "ld a, [hl]",
                    "cp " + CONST_CARD_LOCATION_DECK_VAL, "jr nz, .next", "ld a, l",
                    "call " + FUNC_GET_CARD_ID_FROM_DECK_INDEX_ADDR, "ld bc, " + cardId,
                    "call $3090", // compare DE to BC
                    "jr z, .play_sfx", // found Bellsprout, go back to top loop
                    ".next", "inc l", "ld a, l", "cp " + CONST_DECK_SIZE_VAL, "jr c, .loop_b_press",
            ...
            List<String> aiSelectCode = Arrays.asList("CallForFamilyAIS" + cardName + ":",
                    "call " + FUNC_CREATE_DECK_CARD_LIST_ADDR, "ld hl, " + VAR_DUEL_TEMP_LIST_ADDR,
                    ".loop_deck", "ld a, [hli]", "ldh [$ffa0], a", "cp $ff", "ret z", // no
                                                                                      // Bellsprout
                    "call " + FUNC_GET_CARD_ID_FROM_DECK_INDEX_ADDR, "ld a, e", "cp " + cardId,
                    "jr nz, .loop_deck", "ret" // Bellsprout found
            );
