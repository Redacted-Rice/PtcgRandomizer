...
public class FarCall extends Call {
    ...
    public static FarCall create(String[] args, String rootSegment) {
        ...
        // Otherwise it should be a label
            ...
            if (SegmentNamingUtils.isOnlySubsegmentPartOfLabel(labelOrAddrToGoTo)) {
                labelOrAddrToGoTo = CompilerUtils.formSegmentLabelArg(labelOrAddrToGoTo,
                        rootSegment);
            }
    ...
    protected BankAddress getAddressToGoTo(AssignedAddresses assignedAddresses,
    ...
    public int writeBytes(QueuedWriter writer, BankAddress instructionAddress,
        ...
        if (!foundAddress.isFullAddress()) {
            if (getLabel() != null) {
                throw new IllegalAccessError("FarCall tried to write address for " + getLabel()
                        + " but it is not fully assigned");
            }
            throw new IllegalAccessError(
                    "FarCall tried to write specific address but it is not fully assigned");
        }
    ...
    protected int writeFarJpCall(QueuedWriter writer, BankAddress addressToCall)
            throws IOException {
        // This is an "RST" call (id 0xC7). These are special calls loaded into the ROM at the
        // beginning. For
